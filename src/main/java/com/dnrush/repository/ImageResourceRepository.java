...
public interface ImageResourceRepository extends JpaRepository<ImageResource, Long> {
    ...
    // 分頁查詢方法 - 使用 Spring Data JPA 的 Pageable 替代原生 SQL
    @Query("SELECT i FROM ImageResource i WHERE i.isActive = :status ORDER BY i.year DESC, i.createdAt DESC")
    List<ImageResource> findByStatusOrderByYearDescCreatedAtDesc(boolean status);
    
    @Query("SELECT i FROM ImageResource i WHERE i.category = :category AND i.isActive = :status ORDER BY i.year DESC, i.createdAt DESC")
    List<ImageResource> findByCategoryAndStatusOrderByYearDescCreatedAtDesc(String category, boolean status);
    
    @Query("SELECT i FROM ImageResource i WHERE i.year = :year AND i.isActive = :status ORDER BY i.createdAt DESC")
    List<ImageResource> findByYearAndStatusOrderByCreatedAtDesc(Integer year, boolean status);
    
    @Query("SELECT i FROM ImageResource i WHERE i.category = :category AND i.year = :year AND i.isActive = :status ORDER BY i.createdAt DESC")
    List<ImageResource> findByCategoryAndYearAndStatusOrderByCreatedAtDesc(String category, Integer year, boolean status);
}
